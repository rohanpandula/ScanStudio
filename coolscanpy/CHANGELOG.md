...
- Add keyword-only ``open(..., allow_unverified=False)`` opt-in for the recognized direct-USB LS-40 ED and LS-50 ED identities. The selected bus/address, VID/PID, model, and explicit opt-in travel together through each capture job; the worker refuses missing or replaced identities before capture and records the verified identity in attempt journals. LS-4000/8000/2000 and COOLSCANIII remain discovery-only because this worker has no direct-USB identity for them. No LS-40 or LS-50 has been driven by this driver; these paths are hardware-unvalidated.
- Size the fine READ from ``samples_per_scan``. At 1, the captured record layout shows one RGB/IR group occupying 63,136 bytes with its trailing pad ending at 63,488, so 2,980 READs now request 189,194,240 bytes with CDB transfer bytes ``00 f8 00``. The 2026-09-06 LS-5000 failure had accepted the one-sample window but retained the 207,872-byte four-sample READ and 619,458,560-byte total, then failed with libusb OVERFLOW before completing any bytes. The four-sample plan remains byte-for-byte unchanged. Streaming remains limited to its proven 207,872-byte records; 63,488-byte records use the existing strict offline decoder. This derivation is hardware-unvalidated pending a successful single-sample capture.
