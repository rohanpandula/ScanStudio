...
### `sim.loadMedia`
`{carrier: "roll36"|"strip6"|"mounted", previewFixture?: "textured" | "boundaryAndBlank"}` → `ScannerStatus`, and emits `scanner.status`. Simulator-only affordance (a real backend detects media; that is why the method lives under `sim.`). Frame counts: roll36 → 36, strip6 → 6, mounted → 1. `previewFixture` is itself a **simulator-only test affordance** existing so the manual-review and blank-frame paths (D-18) are exercisable without hardware: `"textured"` gives every subsequently-previewed frame a real, textured raster tile; `"boundaryAndBlank"` additionally flags the second-to-last frame for manual review and gives the last frame a uniform blank tile. Omitting it (or omitting it after a fresh load) leaves the simulator's output exactly as it has always been -- no `imagePath`, no flagged frame. Arming persists until the next `sim.loadMedia` call. Errors: `NOT_CONNECTED`, `SCANNER_BUSY`, `INVALID_PARAMS` (an unrecognized `previewFixture` value).
...
OutputRecipe    {archive: ArchiveRecipe, rawExport: RawExportRecipe, positive: PositiveRecipe, preview: PreviewRecipe,
                 autoCrop: bool}
...
RawExportRecipe {enabled: bool (default false), fileFormat: "linearDng"|"linearTiff",
...
PreviewRecipe   {enabled: bool, fileFormat: "tiff"|"jpeg", maxLongEdgePx: u32,
                 filenameTemplate: string, destination: string}
WrittenOutputs  {archivePath?: string, rawNegativePath?: string, rawNegativeIrPath?: string,
                 positivePath?: string, previewPath?: string,
...
`ScannerStatus.motionArmed` is present only for a real bridge session and mirrors BRIDGE.md's live, no-motion `DeviceStatus.motionArmed` observation. It is informative, not authority: the packaged macOS `ScanStudioLauncher` prepares the session environment and shared authorization latch when it selects a hardware bridge; on Windows/WSL, an ordinary Start-menu or Explorer launch is deliberately unarmed and the separately named `ScanStudio Hardware Session` launcher prepares one attended child session from an explicit media name. The bridge still re-checks both gates at every motion-capable request. Launch itself sends no hardware command; Preview, Scan, and Eject remain explicit user actions. Direct bridge/developer launches remain responsible for their own authorization. The simulator omits the field rather than fabricating a hardware-ready state.

`OutputRecipe.autoCrop` defaults to `false` when omitted. When enabled, each frame's derived positive and preview are cropped independently to the detected image area. The retained archive master and raw negative remain full-frame. `ScanReceipt.autoCrop` records the half-open ROI or the reason the crop was not applied; an approved manual `FrameAlignment` takes precedence, and an unusable detection falls back to the uncropped derivative.
...
`ArchiveRecipe.enabled` defaults to `true` when omitted, preserving older projects. `RawExportRecipe` defaults to disabled when its `rawExport` container is omitted, so old projects never start producing new files. `tiffInfrared` retains its historical wire name and defaults to `fourthChannel`; the additive `sidecar` value applies to either file format. At least one retained output (`archive`, raw negative, positive TIFF, or positive JPEG) is required for every effective frame recipe. A raw-only recipe is valid. When archive retention is disabled, `WrittenOutputs.archivePath` is absent; the real backend still reports bridge provenance in `rgbPath`/`irPath` while routing the mandatory physical capture into an engine-owned private working directory. That temporary capture is cleaned only after the observed bridge terminal closure and successful derivative completion; otherwise it is recovery-held and never represented as a user output.

Raw negative output is always an untouched, full-resolution, unsigned 16-bit export in stored capture orientation. `linearDng` uses a three-sample `LinearRaw` main IFD. Its legacy `fourthChannel` and `omitted` recipe values both retain the established behavior: captured IR is a same-size grayscale SubIFD referenced by a TIFF/EP `LONG` offset and identified by the standard `ImageDescription` value `Untouched Nikon Coolscan infrared plane`; DNG never emits private tag 65001. `linearTiff` uses RGB photometric data; `fourthChannel` interleaves R,G,B,IR with `ExtraSamples=0`, while `omitted` writes RGB only. For either format, `sidecar` keeps the main file RGB-only and writes captured IR beside it as `{main-stem}-ir.tif`, a full-resolution 16-bit BlackIsZero TIFF carrying the established private TIFF marker and DPI. Successful paired receipts record the main under `rawNegativePath` and the grayscale TIFF under `rawNegativeIrPath`. These files are never inverted, color converted, cropped, transformed, or dust-cleaned. The existing Positive output is the path for an IR-cleaned rendered image. When IR was not captured, `sidecar` produces only the same main file as the format's no-IR sibling and leaves `rawNegativeIrPath` absent; TIFF `fourthChannel` still rejects an RGB-only capture.

`ScanReceipt.rgbPath`/`irPath`/`meterRgbiPath`/`hardwareTelemetry` are populated only by a real backend, forwarding BRIDGE.md's `ScanReceipt.rgbPath`/`irPath`/`meterRgbiPath` and `exposure`/`clipping`/`focusDetail`/`transportSmear` telemetry verbatim (mirrored field-for-field under `HardwareTelemetry`'s `ExposureVector`/`ClippingTelemetry`/`FocusDetailTelemetry`/`TransportSmearAssessment`). `rgbPath`/`irPath` are bridge-written capture-source locations, deliberately distinct from the user-facing retained artifacts under `outputs`/`WrittenOutputs`; the raw negative is bridge-written but is still represented only as a retained output, never as a capture source. The simulator omits all four bridge-only source/telemetry fields.
