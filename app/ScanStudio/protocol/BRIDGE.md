...
- `multisamplePasses` must be a member of the opened device's `capabilities.supportedMultisamplePasses`: `[4]` against a CoolscanPy without single-sample support (the traced Nikon Scan capture), `[1, 4]` from CoolscanPy 0.7.7 on, where `1` patches the fine window to one sample per line for a faster, noisier capture. The value is passed to `Roll.scan_many(samples_per_scan=...)`.
