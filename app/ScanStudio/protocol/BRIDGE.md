...
**Armed latch.** Re-checked live — never cached — on every motion-capable request: env `SCANSTUDIO_HW_MOTION == "1"` AND file `~/.scanstudio/hw-motion-armed` exists as a regular file with non-empty stripped authorization text. Either condition failing is `HW_MOTION_NOT_ARMED`. The packaged macOS `ScanStudioLauncher` prepares both halves automatically when it selects a hardware bridge. On Windows/WSL, ordinary Start-menu and Explorer launches are deliberately unarmed; the separately named `ScanStudio Hardware Session` launcher prepares both halves for one attended child process after the owner supplies an explicit media name. On macOS the environment half ends with the app process. On Windows it is scoped to the supervised app/engine tree, but a Linux bridge can outlive its Windows relay; the cleanup guardian therefore attempts ownership-matched latch removal after the supervisor dies, and the next launcher refuses any surviving bridge. A later normal launch still lacks the environment half, so a persistent latch alone does not authorize it. Launching sends no hardware command — `roll.preview`, `scan.start`, and `device.eject` remain explicit actions. Direct bridge/developer launches must prepare their own authorization. There is no method literally named `feed` — `roll.preview` stands in for it, since a preview requires the same physical roll motion as a feed.
...
- `multisamplePasses` must be a member of the opened device's `capabilities.supportedMultisamplePasses`. Production advertises `[4]`. The bridge's source constant `SINGLE_SAMPLE_VALIDATED_RUN` ships as `None`; `[1, 4]` requires an attended validation evidence identifier in that constant and the release notes, plus a driver that declares and accepts `samples_per_scan`. The old `SCANSTUDIO_BRIDGE_SINGLE_SAMPLE` environment variable has no effect. While the gate is closed, the production recipe with `multisamplePasses: 1` returns `INVALID_PARAMS` before motion. The 2026-09-06 first fine READ failed with libusb OVERFLOW and required a power-cycle; simulator tests and derived framing arithmetic cannot unlock this gate.
