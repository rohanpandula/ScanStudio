...
- Optional `recipe.exposureOverride10ns: [R, G, B]` supplies the same RGB authority on every requested frame, including separate scan calls. Requires `autoExposure: false` and three integers in `50000..400000` (raw 10 ns ticks, fixed LS-5000 protocol bounds). Infrared remains metered. Invalid values are refused before reservation or motion. Absent/null preserves the existing per-call behavior.
