...
| `roll save --name … --carrier … --frame-count … --film-process … --confirm-motion [--wait]` / `roll open <directory>` / `roll list` | `roll.save` / `roll.open` / `roll.list` | `--confirm-motion` (`save` only) | 0, 65, 69, 70, 75, 77 (`save`, also a `--wait`ed `failed` terminal state); 0, 65, 69, 70, 75 (`open`/`list`) |
