...
fn physical_target(path: &std::path::Path) -> Result<PhysicalTarget, domain::EngineError> {
    ...
    Ok(PhysicalTarget {
        ...
        #[cfg(unix)]
        file_id: leaf_metadata.as_ref().map(|metadata| (metadata.dev(), metadata.ino())),
...
fn validate_target_candidates(
    ...
    for candidate in candidates {
        ...
        for (other, other_physical) in &resolved {
            ...
            #[cfg(unix)]
            let same_file = physical.file_id.is_some()
                && physical.file_id == other_physical.file_id;
            #[cfg(not(unix))]
            let same_file = false;
...
fn targets_match(
    ...
    #[cfg(unix)]
    let same_file =
        expected.file_id.is_some() && expected.file_id == actual.file_id;
    #[cfg(not(unix))]
    let same_file = false;
...
pub fn validate_frame_output_paths(
    recipes: &domain::OutputRecipe,
    frame_index: u32,
) -> Result<(Option<std::path::PathBuf>, Option<std::path::PathBuf>, Option<std::path::PathBuf>), domain::EngineError> {
