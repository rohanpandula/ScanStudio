...
fn validate_target_candidates(candidates: &[TargetCandidate]) -> Result<(), domain::EngineError> {
    ...
    for candidate in candidates {
        ...
        for (other, other_physical) in &resolved {
            ...
            #[cfg(unix)]
            let same_file =
                physical.file_id.is_some() && physical.file_id == other_physical.file_id;
            #[cfg(not(unix))]
            let same_file = false;
...
fn targets_match(
    ...
    #[cfg(unix)]
    let same_file = expected.file_id.is_some() && expected.file_id == actual.file_id;
    #[cfg(not(unix))]
    let same_file = false;
