...
The issue's “baked in” wording is interpreted as the processed Positive path ScanStudio already has: Digital ICE may use IR there and the negative may be inverted/rendered. It will not be presented as a raw option because calling an IR-cleaned or inverted image “raw” would be misleading. The new linear TIFF choices are therefore only “fourth channel” and “omitted”; the UI will point people who want an IR-cleaned result to Positive.

All three new choices preserve the stored capture orientation recorded by `storageTransform`. They do not crop, rotate, flip, invert, run Digital ICE, apply Nikonlook, assign an ICC profile, rescale, or requantize the real scanner data.
...
Raw export hooks into step 2, while the original `uint16` arrays are still available. This is the narrowest place that can guarantee the promised untouched samples without decoding an intermediate image. The bridge reserves the raw target in the same pre-motion output group as RGB, IR, and meter, writes it before reporting the frame complete, and includes its final path in the bridge receipt. A failed raw write therefore cannot produce a successful frame event. Fully written capture evidence remains recoverable, while an incomplete raw reservation is identity-checked and removed.

The Rust engine owns the user recipe, naming, collision preflight, per-frame overrides, receipt persistence, and private-capture lifecycle. It translates the effective raw recipe into the bridge output plan and records the returned path under `WrittenOutputs.rawNegativePath`. The deterministic Rust simulator writes the same container contracts from its synthetic source raster so simulator-only app and engine tests exercise the option; simulated IR is test data and is always identified by the receipt's existing `simulated` flag.
...
- `tiffInfrared`: `fourthChannel` or `omitted`, default `fourthChannel`. It is ignored for DNG because a DNG export always embeds IR when the capture has IR.
...
The Batch Settings inspector has a **Raw negative** disclosure with a format picker and, for linear TIFF, a conditional infrared picker. Together they expose:
...
RGB-only captures cannot supply IR. Linear DNG will still contain the untouched RGB main image but no IR SubIFD, and the UI will describe IR as conditional on RGB+IR capture. Linear TIFF “RGB + IR” will fail closed if the requested capture receipt has no IR rather than silently emitting three channels under a four-channel choice.
...
- `SubIFDs` (330): one offset when IR exists; absent for RGB-only capture.
...
- Data is encoded through the already-open reservation. A path is marked written only after the encoder and required tag patch finish. Exceptions remove the bridge-owned incomplete reservation and do not emit `scan.frameCompleted`.
- Simulator raw output is encoded into a unique same-directory sibling, flushed and synced, and published create-only. A failed encode leaves no final file.
- Raw target paths are validated against the effective engine recipe before the engine opens sources, persists success, or cleans a private capture.
...
- `make test` at repository root (Rust engine and Swift app through the repository Makefiles, followed by bridge and CoolscanPy suites).
...
- Uncompressed RGB+IR is large. DNG uses separate RGB and IR strips rather than duplicating a four-channel raster in memory, but a full-resolution file is still roughly four 16-bit samples per pixel plus small metadata overhead.
...
- The UI uses a two-choice format picker plus a conditional two-choice infrared picker instead of a single three-choice picker. This follows the wire recipe directly and avoids presenting DNG's embedded-IR behavior as a switchable TIFF property.
