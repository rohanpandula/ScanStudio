...
Raw negative output is always an untouched, full-resolution, unsigned 16-bit export in stored capture orientation. `linearDng` uses a three-sample `LinearRaw` main IFD. Its legacy `fourthChannel` and `omitted` recipe values both retain the established behavior: captured IR is a same-size grayscale SubIFD marked by private ASCII tag 65001 (`scanstudio.infrared.linear.uint16.v1`). `linearTiff` uses RGB photometric data; `fourthChannel` interleaves R,G,B,IR with `ExtraSamples=0`, while `omitted` writes RGB only. For either format, `sidecar` keeps the main file RGB-only and writes captured IR beside it as `{main-stem}-ir.tif`, a full-resolution 16-bit BlackIsZero TIFF carrying the same private marker and DPI. Successful paired receipts record the main under `rawNegativePath` and the grayscale TIFF under `rawNegativeIrPath`. These files are never inverted, color converted, cropped, transformed, or dust-cleaned. The existing Positive output is the path for an IR-cleaned rendered image. When IR was not captured, `sidecar` produces only the same main file as the format's no-IR sibling and leaves `rawNegativeIrPath` absent; TIFF `fourthChannel` still rejects an RGB-only capture.
