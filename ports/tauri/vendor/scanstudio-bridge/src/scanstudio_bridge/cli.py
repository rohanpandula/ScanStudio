...
def main() -> None:
    # BRDG-02: byte-exact NDJSON on every OS. Rewrap stdout once so "\n" is
    # never translated to "\r\n" (Windows text mode would otherwise do this
    # at the OS layer). Guarded so tests that replace sys.stdout with a
    # plain io.StringIO (no .buffer attribute) keep working.
